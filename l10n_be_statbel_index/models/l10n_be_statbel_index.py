...
class L10nBeStatbelIndex(models.Model):
    ...
    display_date = fields.Char(compute="_compute_display_date", store=True, index=True,)
    base_year = fields.Char(required=True, index=True,)
    index_type = fields.Selection(
        selection=lambda self: self._get_index_type(), required=True, index=True,
    )
    ...
    def _compute_display_name(self):
        for record in self:
            record.display_name = "{display_date} ({base_year})".format(
                display_date=record.display_date, base_year=record.base_year,
            )
