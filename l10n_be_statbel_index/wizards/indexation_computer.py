...
class StatBelIndexationComputer(models.TransientModel):
    ...
    reference_index_id = fields.Many2one(
        compute="_compute_base_year", comodel_name="l10n_be.statbel.index",
    )
    actual_index_id = fields.Many2one(
        compute="_compute_price", comodel_name="l10n_be.statbel.index",
    )
    original_price = fields.Float(digits="Product Price", required=True,)
    computed_price = fields.Float(digits="Product Price", compute="_compute_price",)
