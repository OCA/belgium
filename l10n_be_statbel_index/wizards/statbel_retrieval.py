...
class StatBelRetrieval(models.TransientModel):
    ...
    def _get_data(self, res_request):
        ...
        with xlrd.open_workbook(filename, on_demand=True) as xls_file:
            values_by_date_and_ref = dict()
            worksheet = xls_file.sheet_by_index(0)
            row = 1
            while row < worksheet.nrows:
                format_date = self._format_date(
                    worksheet.cell(row, 1).value, worksheet.cell(row, 0).value
                )
                date_ref = str(int(worksheet.cell(row, 9).value))
                ...
                values_by_date_and_ref[format_date][date_ref].update(
                    {
                        "consumer_index": worksheet.cell(row, 2).value,
                        "consumer_index_wo_ene": worksheet.cell(row, 3).value,
                        "consumer_index_wo_ptrl": worksheet.cell(row, 4).value,
                        "inflation": worksheet.cell(row, 6).value,
                        "health_index": worksheet.cell(row, 7).value,
                        "smooth_health_index": worksheet.cell(row, 8).value,
                    }
                )
                row += 1
